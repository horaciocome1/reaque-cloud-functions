...
export async function updateTopicPostsCount(context: functions.EventContext, post: FirebaseFirestore.DocumentData) {
    try {
        console.log(`update topic's posts_count | ${post.topic.id}`)
        ...
        console.log(`failed to update topic's posts count ${context.params.postId}`, err)
    }
}

export async function updateTopicUsersCount(context: functions.EventContext, post: FirebaseFirestore.DocumentData) {
    try {
        console.log(`update topic's users_count | ${post.topic.id}`)
        const db = admin.firestore()
        const snapshot = await db.collection('users').where(`topics.${post.topic.id}`, '==', true).get()
        console.log(`topic's users detected`);
        let count = 0
        snapshot.forEach(_ => count++)
        await db.doc(`topics/${post.topic.id}`).set({users_count: count}, {merge: true})
        console.log(`topic's users count updated | ${count}`)
    } catch (err) {
        console.log(`failed to update topic's users count ${context.params.postId}`, err)
    }
}

export async function updateUserPostsCount(context: functions.EventContext, post: FirebaseFirestore.DocumentData) {
    try {
        console.log(`update user's posts_count | ${post.user.id}`)
        const db = admin.firestore()
...
export async function updateUserFavoriteForCount(context: functions.EventContext, after: FirebaseFirestore.DocumentData) {
    try {
        ...
        console.log('failed to update favorite for count update')
    }
}
